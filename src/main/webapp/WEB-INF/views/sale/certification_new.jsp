...
<html>
	...
	<body>
		<div class="display_none"><button onclick="javascript:window.print()">print1111</button></div>
		<table border="0" width="700">
			...
			<tr>
				<td colspan="9" class="tdcenter" height="28">
					...
					<input class="input_text" type=text value="   ${lotInfo.ARTIST_NAME_EN_TXT}" size="50" style="text-align:left; font-size:13px; font-family:Nanum Gothic; letter-spacing:1px; color:#939393;">
				</td>
			</tr>
			<tr>
				<td colspan="9" class="tdcenter" height="28">
					<c:choose>
						...
						<c:otherwise>
							...
							<input class="input_text" type=text value="   ${lotInfo.TITLE_EN_TXT}" size="50" style="text-align:left; font-size:12px; font-family:Nanum Gothic; letter-spacing:1px; color:#939393;">
						</c:otherwise>
					</c:choose>
				</td>
			</tr>
			...
			<tr>
				...
				(주)서울옥션은 제${fn:trim(saleInfo.SALE_TH)}회 경매(<fmt:formatDate type="both" pattern="yyyy년 M월 dd일"  value="${saleInfo.TO_DT}" />)에서 거래된 상기작품의 기술된 내용을 약관에 따라 보증합니다.<br></span>
				</td>
			</tr>
			<tr>
				...
				We guarantee the described contents of artwork sold on the ${fn:trim(saleInfo.SALE_TH)}${fn:trim(saleInfo.SALE_TH_DSP)} Auction(<fmt:formatDate value="${dateFmt}" pattern="MMM d, yyyy"  timeZone="${zn}"/>) according to the terms and conditions.</span></td>
			</tr>
