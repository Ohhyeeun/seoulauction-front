...
app.controller('ConditionReportListCtl', function($scope, common, ngDialog) {
	...
	$scope.crSave = function($input) {
		<%-- alert 확인/취소 여부 --%>
		var retVal = confirm("컨디션리포트 추가 약관에 동의하십니까?");
		<%-- alert 확인 --%>
		if(retVal == true) {
			var $d = {"baseParms":{"sale_no":$input.sale.SALE_NO, "lot_no":$input.lot.LOT_NO}, 
					"actionList":[
			...
			common.callActionSet($d, function(data, status) {
				if(data.tables["CUST_CR_LOT"]["rows"].length > 0) {
					if($scope.locale == 'ko') {
						alert("컨디션리포트 관심추가가 완료되었습니다.");
						window.location.reload(true);
					}else {
						alert("Your condition report interest has been added.");
						window.location.reload(true);
					}
					return true;
				}else {
		...
		<div id="container">{{a}}
			...
			<div class="contents_wrap">
				<div class="contents">
					...
					<div class="tbl_top">
						<div class="left">
							<div class="txt"><span class="txt_green">{{crListCnt}}</span> <spring:message code="message.mypage.bidding" /></div>
						</div>
						...
						<table style="table-layout:fixed !important;">
							...
							<tbody>
								<tr ng-repeat="cr in crList">
									...
									<td> <button type="button" class="btn_insert" ng-click="crDel({'parent':this, 'crSeq':cr.CR_SEQ, 'sale':cr.SALE_NO, 'lot':cr.LOT_NO});">CR Del.</button></td>
