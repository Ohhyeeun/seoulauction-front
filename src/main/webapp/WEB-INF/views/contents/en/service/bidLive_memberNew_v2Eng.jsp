...
// 		$scope.isZeroBase = false;
		...
		$scope.loadLiveAuction = function(){
			$d = {"baseParms":{"sale_no":$scope.sale_no, "lot_no":$scope.lot_no, "mid_lot_no":$scope.mid_lot_no},
					...
					{"actionID":"get_lotInfo_naviation", "actionType":"select" , "tableName": "LOT_NAVI"},// 낙찰가 표시위해 1회 호출에서 계속 호출로 추가(2021.11.03 YDH)
				]};	 	   	
		...
		var $s = function(data, status) {
			...
			$scope.lotNavi = data["tables"]["LOT_NAVI"]["rows"];// 낙찰가 표시위해 1회 호출에서 계속 호출로 추가(2021.11.03 YDH)
...
// 			$scope.curren = $scope.base_currency; //현재 설정 환율
			...
			// 낙찰가 표시위해 1회 호출에서 계속 호출로 추가(2021.11.03 YDH)
			...
			/* 응찰가 직접입력 처리(2021.10.21 YDH)*/
			var inputPriceUnit = $scope.inputPriceUnitCalc($scope.lot.LAST_PRICE + $scope.lot.GROW_PRICE);
			$("#InputPriceUnit").html(inputPriceUnit);
...
//		 		$scope.bid_price_input_online_KO = "";	
		...
		// 고객응찰가 입력 단위 반환 함수-원화기준(2021.10.21 YDH)
		$scope.inputPriceUnitCalc = function(baseprice){
			var rtnUnit = "";
			var unitWordsKOR = ['', '천', '만', '십만', '백만', '천만', '억', '십억', '백억'];
			var unitWordsENG = ['', 'Thousand', 'Thousand', 'Million', 'Million', 'Million', 'Million', 'Billion', 'Billion', 'Billion', 'Trillon'];
			var splitUnitLength    = 4; // 10,000 자리 기본
			var lenPrice = baseprice.toString().length;
			var lenPriceUnit = Math.floor(lenPrice - splitUnitLength);			

console.log($scope.locale)	;							
console.log(lenPrice)	;
console.log(lenPriceUnit)	;

			if($scope.locale == "en"){
				if (lenPriceUnit > 0)
				{
					rtnUnit = unitWordsENG[lenPriceUnit-1];	
				} else 
				{
					rtnUnit = unitWordsENG[0];	
				}
			
				if ($scope.curren == "KRW") {
					rtnUnit = rtnUnit + " Won";	
				}else {
					rtnUnit = rtnUnit + " Dollar";
				}
			}else{
				if (lenPriceUnit > 0)
				{
					rtnUnit = unitWordsKOR[lenPriceUnit-1];
				} else 
				{
					rtnUnit = unitWordsKOR[0];	
				}					
...
//	 					{"actionID":"get_lotInfo_naviation", "actionType":"select" , "tableName": "LOT_NAVI"},  // 낙찰가 호출로 계속호출로 전환. 주석처리(2021.11.03)
...
//	 	   		$scope.lotNavi = data["tables"]["LOT_NAVI"]["rows"]; // 낙찰가 호출로 계속호출로 전환. 주석처리(2021.11.03)
...
// 			$("#elClickLotNo").val(lotNo);
		...
		// 고객 응찰가 직접입력 로직 추가(2021.10.15 YDH), bid_price는 콤마(,)제거
		$scope.liveLotBidPriceInputSave = function($input) {	
			
			if ($input == 'online_input'){
				$scope.bid_price_input_cust = $("#bidPriceInputCustomer").val();
	 			$scope.bid_price_input = $scope.bid_price_input_cust.replace(/[^\d]+/g,'');
			}else {
				$scope.bid_price_input_cust = 0;
	 			$scope.bid_price_input = 0;
			};
			
			// 현재응찰가 확인 Start(2021.10.15 YDH) //응찰가보다 작은 경우 입력불가. 미입력시 처리 불가. 금액은 (전체dgit-3)까지만 입력
			$scope.bid_price_input_online = $("#bidPriceInputOnline").val();
			$scope.bid_price = $scope.bid_price_input_online.replace(/[^\d]+/g,'');
			
			//if ($scope.bid_price.toString().length < 8){
			//	$scope.bid_price_reset = Math.floor($scope.bid_price/Math.pow(10,$scope.bid_price.toString().length-2));
			//}else{
			//	$scope.bid_price_reset = Math.floor($scope.bid_price/Math.pow(10,$scope.bid_price.toString().length-3));
			//}
			
			// 직접입력 응찰가는 응찰가가 8자리 미만인경우 digit-2, 이상인경우 digit-3으로 단위 환산해서 보여짐.(2021.10.21 YDH확정사항)
			//// 직접입력 응찰가는 응찰가 digit-3 까지만 입력이 가능함. 나머지 자리수는 0으로 설정 (2021.10.21 YDH 주석처리. 입력자체를 단위기반으로 받아서 아래의 로직 사용 안함.)
			//$scope.bid_price_input_reset_tmp = Math.floor($scope.bid_price_input/Math.pow(10,$scope.bid_price.toString().length-3))*Math.pow(10,$scope.bid_price.toString().length-3);
			//$scope.bid_price_input_reset_comma = $scope.commaSetting($scope.bid_price_input_reset_tmp);
			//$scope.bid_price_input_reset = $scope.bid_price_input_reset_comma.replace(/[^\d]+/g,'');
			
			if ($scope.bid_price.toString().length < 8){
				$scope.bid_price_input_reset_tmp = $scope.bid_price_input*Math.pow(10,$scope.bid_price.toString().length-2);
			}else{
				$scope.bid_price_input_reset_tmp = $scope.bid_price_input*Math.pow(10,$scope.bid_price.toString().length-3);
			}			
			$scope.bid_price_input_reset_comma = $scope.commaSetting($scope.bid_price_input_reset_tmp);
			$scope.bid_price_input_reset = $scope.bid_price_input_reset_comma.replace(/[^\d]+/g,'');

			console.log("입력금액: "+$scope.bid_price_input_cust);
			console.log("입력금액: "+$scope.bid_price_input);
			console.log("응찰금액: "+$scope.bid_price_input_online);			
			console.log("응찰금액: "+$scope.bid_price);
			console.log("응찰금액: "+$scope.bid_price_input_reset);
			console.log($scope.lot.LIVE_CLOSE_YN);
			console.log($scope.bid_price_input_reset);
			console.log($scope.bid_price_input_reset_comma);
			
			if ($scope.bid_price == null || $scope.bid_price == ''){
				alert("Bid Ready.");  // alert("현재 준비중입니다.");
			}else if($scope.lot.LIVE_CLOSE_YN =='Y'){
				alert("Bidding is now closed.");  //	alert("응찰 마감한 Lot입니다. 응찰하실수 없습니다.");
			}else if ($scope.bid_price_input == null || $scope.bid_price_input == ''){
				alert("Please fill out the bid amount."); //alert("응찰금액을 입력하세요!");  //
			//}else if (parseInt($scope.bid_price_input) <= parseInt($scope.bid_price)){
			//	alert("현재 응찰가 보다 커야 합니다.");  //alert("The bidding price should be larger than the current price.");
			}else if (parseInt($scope.bid_price_input_reset) <= parseInt($scope.bid_price)){
				alert("The bidding price should be larger than the current price."); // alert("현재 응찰가 보다 커야 합니다.");  //
			}else if (parseInt($scope.bid_price_input_reset) > parseInt($scope.bid_price)){
				if ($scope.lot.IS_WIN == "Y"){
					alert("Highest bidding.");  // alert("최고가 응찰중입니다."); 
				}else
				{ 
					//if(confirm("입력 금액: "+$scope.curren+" "+$scope.bid_price_input_cust+'\r\n        ('+$scope.numberToKorean($scope.bid_price_input) +')\r\n'+
					//		   "응찰 금액: "+$scope.curren+" "+$scope.bid_price_input_reset_comma+'\r\n        ('+$scope.numberToKorean($scope.bid_price_input_reset) +')\r\n'+
					//		   "위 응찰 금액으로 응찰하시겠습니까?") == true){
					if(confirm("Bid: "+$scope.curren+" "+$scope.bid_price_input_reset_comma+'\r\n        ('+
								   "Would you like to bid with the above amount?") == true){	
						var $d = {"baseParms":{"sale_no":$scope.sale_no, "lot_no":$scope.lot_no, "bid_price":$scope.bid_price_input_reset, "bid_kind_cd":"online", "padd_no" : $scope.custInfo.PADD_NO}, 
								"actionList":[
								{"actionID":"addOffBidPrice", "actionType":"insert", "tableName":"BID_OFFLINE", "parmsList":[{}]}
							]};
						common.callActionSet($d, function(data, status) {
							//직접입력 null 처리
							$scope.bid_price_input_customer = '';			
				 			$("#bidPriceInputCustomer").val('');
		...
		}; // 현재응찰가 확인 End (2021.10.15 YDH)
		
		// Lot Refresh : 1초단위, Navi Refresh : 30초단위
		$interval(function(){$scope.loadLiveAuction();},1000);     //2021.10.21 YDH 테스트로 주석 처리
...
/* 			var hanA = new Array("","일","이","삼","사","오","육","칠","팔","구","십"); 
			...
			if(number != 0) result = result + result_unit; 
		    ...
		    for (var i = 0; i < resultArray.length; i++){
		        ...
		        resultString = String($scope.commaSetting(resultArray[i])) + unitWords[i] + resultString;
		    }
		    
		    if ($scope.curren == "KRW") {
		    	resultString = resultString + " 원";	
			}else {
				resultString = resultString + " 달러";
			}
		    return resultString;
		} 
...
<%-- 								</c:choose>   --%> 
		...
		<!-- ng-controller="liveAuctionCtl" data-ng-init="init()" -->
		...
		<div><!-- <div ng-if="lot.LOT_NO != null"> --><!-- 경매 진행중인 경우 -->          
	    <!--main -->    
		<div class="client_onepcssgrid-1200 clearfix">     
			<div class="bidlive_resizeFull clearfix">  
				...
				<div class="bidnav_tapname">  
					<ul> 
							<!-- 카테고리작업 보류
 							<li ng-repeat="cate in category" >   
 								<span ng-if="locale=='ko'" ng-click="cate_toggle(cate.CD_ID, cate_selected)" checklist-model="cate_cds" class="list-category-b"><a href="#" >{{cate.CD_NM}}</a></span> 
                                    <span ng-if="locale!='ko'" ng-click="cate_toggle(cate.CD_ID, cate_selected)" checklist-model="cate_cds" class="list-category-b"><a href="#" class="list-category-b">{{cate.CD_NM_EN}}</a></span>  
							</li> -->
 							<li class="bidnav_tapOn" data-tab="navitab01">List</li>                      
<!-- 							<li class="bidnav_tapOn" data-tab="navitab01">근현대</li>      -->
<!-- 							<li data-tab="navitab02">고미술</li>      -->
					</ul>               
				</div>  
				...
				<div class="bidlive_navigationbox">
					<div class="bidlive_navigationblock" id="naviDiv">
						...
						<div class="naviImgbox on" id="navitab01">                     
							<div class="bidnav_naviimg" ng-class="{'nowLot' : lotNavi.LOT_NO == lot.LOT_NO }" ng-repeat="lotNavi in lotNavi" ng-click="elClick(lotNavi.LOT_NO, lotNavi.STAT_CD, lotNavi.ARTIST_NAME_JSON[locale], lotNavi.TITLE_JSON[locale], lotNavi.MATE_NM_EN , lotNavi.EXPE_PRICE_FROM_JSON[base_currency], lotNavi.EXPE_PRICE_TO_JSON[base_currency], lotNavi.EXPE_PRICE_INQ_YN, lotNavi.FILE_NAME, lotNavi.FILE_PATH )">
								...
								<div ng-if="lotNavi.STAT_CD == 'entry'" class="bidnav_lotinfo">   
									...
									<p clase="textshort priceInfo" ng-if="lotNavi.LOT_NO < lot.LOT_NO">
										<span ng-if="locale!='en'">낙찰가 </span><span ng-if="locale!='ko'">Hammer </span> 
										<span ng-bind="lotNavi.EXPE_PRICE_FROM_JSON[base_currency] | currency : base_currency + ' ' : ''"></span>
										<span ng-bind="lotNavi.MAX_BID_PRICE"/>
									</p>
									<p class="textshort priceInfo" ng-if="lotNavi.LOT_NO >= lot.LOT_NO && lotNavi.EXPE_PRICE_INQ_YN != 'Y' && (lotNavi.EXPE_PRICE_TO_JSON[base_currency]) != 0 && (lotNavi.EXPE_PRICE_FROM_JSON[base_currency]) != 0 && (lotNavi.EXPE_PRICE_TO_JSON[base_currency]) != null && (lotNavi.EXPE_PRICE_FROM_JSON[base_currency]) != null" title="{{lotNavi.EXPE_PRICE_FROM_JSON[base_currency] | currency : base_currency + ' ' : ''}} ~ {{lotNavi.EXPE_PRICE_TO_JSON[base_currency] | number : ''}}"> 
		    							<span ng-bind="lotNavi.EXPE_PRICE_FROM_JSON[base_currency] | currency : base_currency + ' ' : ''"></span> 
						...
						<!-- 고미술 탭 네비게이션 -->  
						<%-- <div class="naviImgbox" id="navitab02">             
							<div class="bidnav_naviimg" ng-class="{'nowLot' : lotNavi.LOT_NO == lot.LOT_NO }"  ng-repeat="lotNavi in lotNavi" ng-click="elClick(lotNavi.LOT_NO, lotNavi.STAT_CD, lotNavi.ARTIST_NAME_JSON[locale], lotNavi.TITLE_JSON[locale], lotNavi.MATE_NM_EN , lotNavi.EXPE_PRICE_FROM_JSON[base_currency], lotNavi.EXPE_PRICE_TO_JSON[base_currency], lotNavi.EXPE_PRICE_INQ_YN, lotNavi.FILE_NAME, lotNavi.FILE_PATH )">   
								<div class="img_navigation">      
									<img ng-if="lotNavi.STAT_CD == 'entry'" oncontextmenu="return false" ng-src="<spring:eval expression="@configure['img.root.path']" />{{lotNavi.FILE_NAME | imagePath1 : lotNavi.FILE_PATH : 'detail'}}" 
										alt="{{lotNavi.TITLE}}" />      
								...
								<div ng-if="lotNavi.STAT_CD == 'entry'" class="bidnav_lotinfo">   
							        ...
							        <!-- live이미지  --> 
							        <p style="margin-bottom: 5px;">    
							        	<span class="lotNo" ng-bind="lotNavi.LOT_NO"></span> 
							        	<span ng-if="lotNavi.LOT_NO == lot.LOT_NO"  class="navi_liveicon"></span>  
							        </p> 
									...
									<p class="textshort priceInfo" ng-if="lotNavi.EXPE_PRICE_INQ_YN != 'Y'" >
			...
			<div class="bidlive_boxCenter clearfix">  
							...
							<table class="bid_history_table"> 
								...
								<tbody style="text-align:center;"> 
									<tr ng-repeat="myBidHistory in myBidHistory">
										...
										<td ng-if="myBidHistory.LAST_BID_PRICE == myBidHistory.HAMMER_BID_PRICE"></td> <!-- 낙찰 -->
									</tr>
								</tbody> 
							</table> 
						</div> 
				...
				<div class="bidlive_boxRight clearfix">     
					<div class="bidlive_Rtop">
						...
						<!-- 현재 진행 응찰 정보-->
						<div class="bidlive_Rcaption_center">      
						...
						<div class="Rcaption_price clearfix">     
							...
							<div class="Rcaption_Bidbtnbox clearfix">                  
								<!-- 응찰버튼 조건 : 정회원 이거나 국외회원 응찰허용 인경우, 메이저/홍콩/기획경매만 응찰버튼 활성화 && (custInfo.MEMBERSHIP_YN == 'Y' || custInfo.FORE_BID_YN == 'Y') &&--> 
								<!--                 사전신청 하여 패들번호 있는 회원의 경우 -->
							...
							<!-- 고객 응찰가 입력 추가(2021.10.15 YDH) -->   
							<div class="bidlive_cust_price_box" ng-if="custInfo.CUST_NO > 0 && custInfo.PADD_NO > 600 && lot.IS_WIN == 'N' && lot.LIVE_CLOSE_YN !='Y'">   
								<div class="bidlive_cust_tit_top"> 
									<label class="bidlive_cust_tit" for="bidPriceInputCustmoer">            
										Bid Amount
									</label>     
									<input class="bidlive_cust_txt" type="text" ng-model="bid_price_input_customer" id="bidPriceInputCustomer" name="bidPriceInputCustomer" onkeyup="getNumber(this)" placeholder="Please fill out the bid amount."/>
									<span id="InputPriceUnit" class="cst_pricetit">        
										<!--<span class="lang_unit" ng-if="curren == 'KRW' && "> 
											{{inputPriceUnit}}                                               
										</span> 단위 -->  
										<!--<span class="lang_unit" ng-if="curren != 'KRW' && ">   
											{{inputPriceUnit}}                                              
									...
									<button class="bidlive_cust_btn" type="button" ng-click="liveLotBidPriceInputSave('online_input');">  
										<span ng-if="locale=='ko'">응찰하기</span>    	
								  		<span ng-if="locale!='ko'">Bid</span>  
								  	</button> 
									<!-- &nbsp;
									<span class="btn_style01 gray02 bidlive_btn"><button type="button" ng-click="liveLotBidPriceInput(-1);">-</button></span>
									<span class="btn_style01 green02 bidlive_btn"><button type="button" ng-click="liveLotBidPriceInput(+1);">+</button></span>  --><!-- 고객 응찰가 입력 추가 end -->
								</div> 
							</div><!-- //bidlive_cust_price_box -->     
